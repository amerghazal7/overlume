...
TEST(Ribbon, WidthChangeRebuildsGeometry) {
    ...
    overlume::RenderConfig cfg{320, 240, /*quality=*/1, fixtureDir.c_str(), "ribbon_width_a"};
    auto* r = overlume::create_renderer(cfg);
    if (!r) GTEST_SKIP() << "no GPU/EGL";
    ...
    ASSERT_TRUE(overlume::set_theme(r, "ribbon_width_b", /*at_sec=*/0.0, /*transition_sec=*/0.2));
    s.sim_time_sec = 0.2;
    overlume::set_scene(r, s);
    render_once(r, pose);
...
TEST(Ribbon, MarginChangeRebuildsGeometry) {
    ...
    // covers.
    const std::string fixtureDir = std::string(OVERLUME_TEST_DATA_DIR) + "/tests/fixtures/themes";
    overlume::RenderConfig cfg{320, 240, /*quality=*/1, fixtureDir.c_str(), "ribbon_margin_a"};
    auto* r = overlume::create_renderer(cfg);
    if (!r) GTEST_SKIP() << "no GPU/EGL";
    ...
    ASSERT_TRUE(overlume::set_theme(r, "ribbon_margin_b", /*at_sec=*/0.0, /*transition_sec=*/0.2));
    s.sim_time_sec = 0.2;
    overlume::set_scene(r, s);
    render_once(r, pose);
