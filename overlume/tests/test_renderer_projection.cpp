...
TEST(ProjectToScreen, PointAtCameraTargetProjectsNearCenter) {
    ...
    const overlume::CameraPose pose = HelloFramePose();
    ...
    const overlume::Vec3 target{pose.target[0], pose.target[1], pose.target[2]};
    float x = -1.0f, y = -1.0f;
    ASSERT_TRUE(overlume::project_to_screen(r, target, &x, &y));
