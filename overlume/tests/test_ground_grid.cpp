...
TEST(GroundGridGolden, TwoLayers_OffroadLightClay) {
    ...
    double ssim = mpviz::testing::render_and_compare(
        r, pose, MPVIZ_TEST_DATA_DIR "/tests/goldens/ogm_offroad_light_clay.png",
...
TEST(GroundGrid, TextureIsUpdatedInPlaceNotRecreated) {
    ...
    render_once(r, pose);
    const void* handleBefore = mpviz::testing::ground_grid_texture_handle(r, 0);
    ...
    mpviz::set_scene(r, s);
    render_once(r, pose);
    const void* handleAfter = mpviz::testing::ground_grid_texture_handle(r, 0);
...
TEST(GroundGrid, DimensionChangeRecreatesTheTexture) {
    ...
    mpviz::set_scene(r, s);
    render_once(r, pose);
    const uint32_t genAfter = mpviz::testing::ground_grid_texture_generation(r, 0);
...
TEST(GroundGrid, NoRedundantUploadWithoutNewIngest) {
    ...
    render_once(r, pose);
    render_once(r, pose);
    EXPECT_EQ(mpviz::testing::ground_grid_texture_upload_count(r, 0), 1u)
    ...
    mpviz::set_scene(r, s);
    render_once(r, pose);
    EXPECT_EQ(mpviz::testing::ground_grid_texture_upload_count(r, 0), 2u)
...
TEST(GroundGrid, StaleGridFadesViaSharedStalenessAlpha) {
    ...
    const float alpha = mpviz::testing::ground_grid_material_alpha(r, /*kind=*/0);
...
TEST(GroundGrid, MaterialIsThemedOnFirstDataWithNoTransition) {
    ...
    mpviz::GroundGridLayer layers[2]{};
    layers[0].kind = 0;
    ...
    const auto freeColor = mpviz::testing::ground_grid_free_color(r);
    ...
    const auto occupiedColor = mpviz::testing::ground_grid_occupied_color(r);
    ...
    EXPECT_FLOAT_EQ(mpviz::testing::ground_grid_material_alpha(r, 0), 1.0f);
    EXPECT_FLOAT_EQ(mpviz::testing::ground_grid_material_alpha(r, 1), 1.0f);
    mpviz::destroy_renderer(r);
}
