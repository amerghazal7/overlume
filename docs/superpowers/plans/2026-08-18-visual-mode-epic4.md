...
| 2 `bake_environment.py`: OSM footprints → chunked, georeferenced glTF + index + verification overlay | VM-051 | **Done 2026-09-10** | All 7 steps landed at `44ed5c3`. Two deviations from the Files/Steps text: (a) Step 5's three height-fallback cases use synthetic in-memory footprints rather than extending the committed Overpass fixture — no way to carry height/`building:levels` tags into the real fixture without fabricating tags into a real-provenance source, rejected per the note already recorded in `assets/models/environment/ATTRIBUTION.md`; (b) Step 4's ego-track polyline reuses Task 1's existing `test/fixtures/geo_anchor_samples_0.csv` via an optional `--ego-track` flag instead of committing a second track fixture. |
