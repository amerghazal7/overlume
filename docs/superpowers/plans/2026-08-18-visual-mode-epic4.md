...
| 3 Runtime chunk load + distance culling behind an `EnvironmentSource` seam, clay building material | VM-052 | **Not started** | — |
...
**Interfaces:**
- Produces (node-internal — this header adds nothing to the `visual_renderer` public API itself, it only consumes `mpviz::GeoAnchor` from `scene.h`, the SAME type Task 3 appends there; no local duplicate, no converter needed anywhere):
```cpp
// geo_anchor.hpp
...
- [ ] **Step 0: Failing round-trip test, pure math, no ROS.** `test_geo_anchor.cpp`:
```cpp
...
**Files:**
- Modify: `cuda/src/libs/visual_renderer/include/visual_renderer/scene.h` (append `GeoAnchor`, `set_environment_source` — Decision 2; bump `kSceneVersion` 3 → 4 for the appended struct, per ADR-0004)
- Modify: `cuda/src/libs/visual_renderer/tests/test_scene_buffer.cpp` and `cuda/src/ros_apps/src/micropilot_visualization_node/test/test_scene_layout.cpp` (bump `static_assert(mpviz::kSceneVersion == 3, ...)` to `== 4` in both; no new `sizeof`/`offsetof` asserts needed — no layout change to any existing struct)
