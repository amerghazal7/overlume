...
- [x] **Step 6: Commit** `feat(visual): bake_environment.py — OSM footprints -> chunked map-frame glTF + index (VM-051)`. **Done 2026-09-10.** Landed at `44ed5c3`.
...
- [ ] **Step 0: Library-internal `EnvironmentSource` seam, no chunks loaded yet.** Failing test, `tests/test_environment.cpp`:
```cpp
...
- [ ] **Step 1: Load the committed test-town fixture; assert chunks appear as renderables when ego is near.** Failing test:
```cpp
...
- `render_ms` delta measured with the test town loaded vs. not, at the shipped default preset: ____ ms (AC bar: < 2 ms)
