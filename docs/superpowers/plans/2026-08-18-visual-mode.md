...
- [x] **Step 4: Run full existing test suite + smoke — PASS.** `visual_renderer` ctest 3/3; `micropilot_visualization_node/test/smoke_test.py` PASS (92 frames @ initial_mode=3, 0 frames @ initial_mode=1); `micropilot_rendering_node/test/smoke_test.py` PASS including the new mode-mux step (3→2→3→1, each switch <0.5s, no gap >0.5s once settled). Full `colcon_build.sh` (all packages) green. **Commit** `feat(mux): global /rendering/set_mode across both nodes` (`<hash filled after commit>`).
