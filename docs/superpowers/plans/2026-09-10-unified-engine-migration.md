...
**Addendum — per-tick extrinsics is real, and the static bake alone does not cover it.** `rendering_node.cpp:735-793` calls `reprojector_->set_cameras(scaled_params)` on **every** render tick, not on config-change: each camera's extrinsics is advanced by `compensate(cam_params_[i], up_stamps_[i], t_max)` whenever odometry is flowing (the ego-motion fix that exists because ~80 ms of camera phase spread bakes ~0.5 m of travel into a stitched frame while driving), per the source's own comment at that call site — *"the GUI tunes extrinsics live even while the image set is frozen ... the vcam/K may change any tick."* A full CPU vertex re-bake every tick (6 cameras × the bowl's vertex count × the projection+distortion math) is not the "expensive, rare operation" this plan can assume. **Chosen mechanism:** keep Decision 3's per-vertex UV/weight bake static (driven only by shape + base extrinsics/intrinsics, as designed above), and carry the ego-motion delta as a small per-camera 4×4 transform uniform that the vertex shader applies to the baked ray before the UV lookup — i.e. the per-tick-varying component of projection moves shader-side, which is precisely the "true per-pixel/per-vertex shader-side projection" alternative Decision 2 already flags as a USER DECISION at Task 2's review gate; that review gate now also decides whether ego-motion compensation is the thing that forces the shader-side path, not just fidelity. The K-rescale half of `rendering_node.cpp:762-767` (output/native resolution ratio) does **not** carry over: it exists only because the CUDA kernel resizes all six camera images onto one fixed NHWC canvas before projecting; this design samples each camera's texture at its native resolution via a normalized UV, so there is no output canvas to rescale K against. Task 2 Step 5's perf gate must include a "bowl + ego-motion uniform update, actively driving" case, not just a static bowl-on/bowl-off pair, and a live GUI extrinsics/bowl-radius slider drag is budgeted as a full re-bake hitch, acceptable up to one dropped frame (~33 ms) at the slider's own update rate — tighter than that requires the shader-side path above, not a silent tessellation tweak.

**bowl.mat's vertex→fragment interpolant budget is a hard Filament limit, not a tuning knob.** `MaterialBuilder.h:214-220` fixes `MATERIAL_VARIABLES_COUNT = 4` (`Variable::CUSTOM0..CUSTOM3`) — 4 custom vec4 interpolants, 16 floats total, per material. Decision 3's naive per-camera (u, v, weight) × 6 cameras is 18 floats — over budget by 2. (Vertex-*attribute* capacity is not the constraint: `MAX_CUSTOM_ATTRIBUTES = 8`, `MaterialEnums.h:158`, and `MAX_VERTEX_ATTRIBUTE_COUNT = 16`, `backend/DriverEnums.h:99`, both comfortably cover 18 floats of attributes — the wall is specifically the 4-variable *interpolant* budget between vertex and fragment stages.) **Packing scheme:** 6 cameras' UVs pack into 3 `vec4`s (`CUSTOM0`=cam0.uv+cam1.uv, `CUSTOM1`=cam2.uv+cam3.uv, `CUSTOM2`=cam4.uv+cam5.uv), and all 6 blend weights quantize into the 4 components of `CUSTOM3` by packing two 8-bit weights per float via `packHalf2x16`-style bit-packing (or, more simply, drop to a per-vertex *dominant-camera* weight scheme if 6-way blending at every vertex proves unnecessary against the golden — an implementer tessellation/blend-count call, same as the fidelity call above). Task 2 Step 3 (the Filament-free bake test) must assert the packed layout round-trips correctly, and a **new Task 2 step compiles `bowl.mat` through `matc` (`-a opengl -p desktop`, the same invocation `visual_renderer/CMakeLists.txt:133` already uses) before any mesh-generation code is written**, so an interpolant-budget violation is caught by the build, not by a later visual artifact. That step also verifies `Texture::isTextureFormatSupported(engine, InternalFormat::RGB8)` (`Texture.h:89`) for the camera textures, falling back to RGBA8 if unsupported — RGB8's row alignment is a known soft spot on some GL backends.

**Exposure matching and blind-zone fill are deferred out of this migration, not silently dropped from `BowlConfig`.** Both are real, default-on algorithms in the CUDA node — `exposure_match` (default `true`, `rendering_node.cpp:88`, and `true` in the deployed `default_params.yaml`) collects GPU luminance statistics where neighboring cameras overlap and solves per-camera gains on the host in log space with a ridge anchor and 0.8/0.2 EMA smoothing, applied the next frame (`reprojector.cpp:97-181,436-444`); `fill_blind_zone` (default `true`, `rendering_node.cpp:79`, `fill_blind_zone: true` in `default_params.yaml`) propagates surrounding scene color into the uncovered ring around the robot before the robot overlay (`kernels/fill.cu`). Neither has a Filament-side implementation in this plan — the merged node's bowl would otherwise silently claim both capabilities (they are named fields on `BowlConfig`, Task 1) while actually rendering a zero-weight blind ring and un-matched seam brightness steps. **Decision: ship both flags declared but forced to `false`** in `micropilot_visualization_node`'s shipped `default_params.yaml` (Task 2), regardless of what the CUDA node's own config carries, so the merged node cannot report a capability it does not have. Record both as **named, user-visible parity exceptions** in Task 4 Step 1's checklist (visible seam-brightness steps between cameras; a visibly-empty ring around the robot instead of an inward-extrapolated fill) — not blocking parity sign-off unless the user says otherwise at that gate, since re-implementing either is materially new library rendering work (a stats pass + host solve, or a skirt mesh / screen-space extrapolation pass) that this epic's own risk-ranking (Decision 1) defers past first cutover.
...
- Modify: `cuda/src/ros_apps/src/micropilot_visualization_node/config/default_params.yaml` (append, mechanically re-derived from `grep -n declare_parameter cuda/src/ros_apps/src/micropilot_rendering_node/src/rendering_node.cpp` — 25 hits, every one asserted covered by this task + Task 5's `pointcloud_transform`: `bowl_enabled: false` — default OFF until parity sign-off, per the STANDING directive's disable-knob rule — plus `n_cameras`, `image_topics`, `info_topics` (these are HOW the node finds the six real cameras and their `CameraInfo`; without them the merged node subscribes to nothing), `max_sync_latency` (the frame-sync gate's threshold, GUI-tunable), `odom_topic` (gates ego-motion compensation, Decision 3's addendum), `config_path` (required by the GUI bridge's config-save path, see below), `bowl_R0`/`bowl_k`/`bowl_Rmax`/`feather_margin`/`fill_blind_zone` (declared, forced `false` per Decision 3's exposure/blind-zone deferral)/`exposure_match` (declared, forced `false`, same deferral)/`sky_color`/`camera_extrinsics` — same param shapes `rendering_node.cpp:53-96,158-205,263-265,366` already declares, ported not reinvented. **`camera_intrinsics` is deliberately NOT added as a param** — intrinsics/distortion are not a ROS parameter anywhere in the CUDA node (see Global Constraints); they are wired via the `CameraInfo` topic callback in `camera_ingest.cpp` below, not `declare_parameter`.)
- Modify: `cuda/src/ros_apps/src/micropilot_visualization_node/src/visualization_node.cpp` (own a `CameraIngest`; add the `add_on_set_parameters_callback` live-tuning handler `rendering_node.cpp:370`'s `on_params` implements, since the GUI's bowl-tuning panel writes through it; on `bowl_enabled` true, call `set_bowl_config()` for the first time once `camera_ingest.cpp` reports every configured camera's `CameraInfo` has arrived — NOT once at `on_activate()` — and re-call it (full re-bake) whenever a camera's `K`/`dist`/dims change thereafter; call `set_camera_frame()` per arriving, dirty camera image)
...
- [ ] **Step 5: Perf gate — real numbers, not just "record the delta."** Measure `render_ms` (VM-034 instrumentation, already exists) and `image_hz`/GPU SM % (`budget_probe.sh`'s existing `nvidia-smi dmon` procedure) with three cases at 1280×720, quality 1 (medium): `bowl_off` (baseline, should match `budget_probe.md`'s existing `q1_alone` row: `image_hz` 30.3, GPU SM 35%, viz CPU 39.0%), `bowl_on_static` (bowl configured, no ego-motion uniform updates), and `bowl_on_driving` (bowl + the Decision 3-addendum per-tick ego-motion uniform actively updating, odometry flowing). **Pass/fail:** `image_hz >= 30.0` in every case (the fixed-33ms-publish-timer rule `budget_probe.md:31-34` already states — a case whose `image_hz` falls below 30 is over budget), GPU SM % recorded as a delta over `q1_alone`'s 35% baseline (itself already ~19 points over the documented 16% desktop-idle floor, `budget_probe.md:43`), and `render_ms` p99 `<= 33 ms`. If `bowl_on_driving` misses either bar while `bowl_on_static` holds, that isolates the ego-motion per-tick cost specifically and is the review-gate finding that triggers the vertex-baking-vs-shader-sampling USER DECISION named in Decision 2/3, not a silent tessellation tweak.
