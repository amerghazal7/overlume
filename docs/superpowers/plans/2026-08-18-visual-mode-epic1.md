...
- [ ] **Step 12: Commit** `feat(visual): ego robot from M02P glTF, TF-driven pose + smoothed speed, clay-box fallback (VM-012)`.
  ...
  **Status (corrected 2026-08-20 — this block was stale):** DONE — committed as `e51bad6` (`feat(visual): ego robot from M02P glTF, TF-driven pose + smoothed speed, clay-box fallback (VM-012)`), and that commit also folded in the live additions from the 2026-08-19 validation session (Task 5's ego-anchored-camera composition and the GUI 3-way view cycle documented in that task's scope-addition blocks above) rather than leaving them for Task 5's own commit. Step 2 — the manual M02P.obj→glTF conversion against the user's real Downloads asset — is still pending: it's a one-time manual step against a non-git asset, not something any handoff can do on the user's behalf.
