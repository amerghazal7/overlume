...
**Parent plan:** `docs/superpowers/plans/2026-08-18-visual-mode-epic3.md` — Epic 3 is CLOSED at `af628df`; this plan is new-stack follow-on work staged there under "New-stack intake, PENDING user 'go' (2026-09-09) — VM-077" (that section's step 4: "Author the new-topic rendering work as its own scoped task(s) AFTER inspecting the recorded data"). Its "STANDING user directive 2026-09-09 — every rendered element ships with style + disable config" binds every task below.
...
### 2026-09-10 — BEHAVIOR-ribbon flicker ROOT CAUSE proven and fixed: blended-queue draw-order dependence

**The still-open verdict above is now closed.** The staleness-fade theory was ruled out (the observed pattern is a binary 0.0274→0.0 toggle, not a ramp; the anchored-stamp fix landed anyway and stands on its own). The proven mechanism: `ribbon_emissive.mat` (BEHAVIOR) and `trajectory_carpet.mat` (velocity ribbon) were BOTH `blending : fade`, so both renderables lived permanently in Filament's blended queue — per-renderable sorted, **no depth writes between blended draws**. Co-located on one spine (the respine makes them exactly coincident laterally, 6 mm apart in z), whichever strip was drawn LAST fully overpainted the other at alpha 1 — and a content rebuild re-enters the queue, so the ~8 Hz alternation of behavior-path and carpet publishes flipped the draw order back and forth. The z-stagger was irrelevant in the blended pass: no depth write means z never arbitrates.

**Deterministic repro (fails pre-fix, passes post-fix):** `tests/test_ribbon_dropout.cpp` `BehaviorRibbonNeverVanishesUnderCoLocatedCarpetChurn` — one BEHAVIOR ribbon + one red carpet on the same spine, alternating tail-nudge rebuilds every 4 frames (the live publish pattern), teal-pixel count probed per frame. Pre-fix: teal present on exactly 16/32 frames, flipping precisely at each rebuild boundary — the live burst's binary pattern (15/24), reproduced single-threaded. The pre-existing dropout tests missed it because their `looks_like_corridor` probe is warm-only (`r > b`) — blind to teal.

**Fix — opaque-when-fresh, the shape GLOBAL/LOCAL always had (and why they never flickered):**
- `ribbon_emissive.mat`: `blending : fade` + `depthWrite : false` removed — OPAQUE. Depth writes make the z-stagger (BEHAVIOR 0.058 over velocity 0.052) decide per-pixel, draw order irrelevant. `baseColor` stays float4 (alpha ignored) so no renderer.cpp signature churn.
- BEHAVIOR staleness now fades via the `clay_translucent.mat` instance swap — `ribbon.cpp`'s role branches collapse into one shared path. Emissive glow is not carried through the 1 s death-fade (accepted: both shipped themes author `emissive.ribbon_strength` 0.0).
- `trajectory_carpet.mat`: OPAQUE, alpha uniform removed. New death-fade twin `trajectory_carpet_faded.mat` (the old fade shader verbatim: unlit + per-vertex color + alpha uniform) — `clay_translucent` can't serve (LIT needs tangents CarpetVertex lacks; no COLOR input). `trajectory_carpet.cpp` swaps the whole layer's meshes between the two shared instances by `staleness_alpha()`; `build_slot_meshes()` resets to opaque (`TrajectoryCarpetSlot::boundFaded`).
- `test_ribbon.cpp` `StaleRibbonFadesViaSharedStalenessAlpha`: BEHAVIOR expectation flipped to `bound_to_translucent == true`.

**Verification:** library 158/158 (156 prior + repro test + the one ctest -j4 perf flake `Objects.FiftyObjectsSceneUpdateUnderTwoMilliseconds` passes standalone twice — GPU contention, not a regression); node force-relinked, 221 assertions green. Live after-fix burst evidence: **user-approved 2026-09-10 ("Flicker gone!")**, promoted to `docs/evidence/vm077-flicker-2026-09-10/opaque_fix_burst_after{.gif,_summary.json,_stddev_heatmap.png,_full_0.png}` — teal present 24/24 frames at a constant ~0.0274 fraction (pre-fix: exactly 0.0 on 15/24). The flicker saga is CLOSED.
