...
```
  ...
  - `[review 2026-09-07]` The block above is the ORIGINAL api.h-only script. The shipped `scripts/check_pod_header.sh` (generalised in 9cb0b2d when `scene.h` landed) globs every file under `include/visual_renderer/` with a non-empty guard, so `scene.h` is covered. Glob is `*.h`; a public `.hpp` would escape — VM-037 widens it.
...
- [x] **Step 5: Commit** `feat(visual): headless Filament hello-frame behind POD API` (`c71847e`).
...
- [x] **Step 4: Run smoke — PASS.** `initial_mode=3`: 92 frames in 3 s, `rgb8` `320x240`. `initial_mode=1`: 0 frames in 2 s. **Commit** `feat(visual): visualization node streaming hello-frame on mode 3` (`1484cd7`).
...
- [x] **Step 4: Run full existing test suite + smoke — PASS.** `visual_renderer` ctest 3/3; `micropilot_visualization_node/test/smoke_test.py` PASS (92 frames @ initial_mode=3, 0 frames @ initial_mode=1); `micropilot_rendering_node/test/smoke_test.py` PASS including the new mode-mux step (3→2→3→1, each switch <0.5s, no gap >0.5s once settled). Full `colcon_build.sh` (all packages) green. **Commit** `feat(mux): global /rendering/set_mode across both nodes` (`9fc3000`).
...
**Epic 2 CLOSED at c5ea38c (2026-09-07)** — gate PASSED 2026-08-20 (26f17f0) with user-authorized deviations; post-gate: `flatten_z` param (93f62f8), three ribbon theme tokens (ffa943a), gt-boxes + `/road_markers` rows disabled (c5ea38c). Status ledger in the Epic 2 plan.
