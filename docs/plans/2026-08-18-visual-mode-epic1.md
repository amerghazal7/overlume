...
**Epic 1: CLOSED at `e47b057` (gate round 1 applied), review-approved.** Every step
...
|---|---|---|---|
| 1 SceneGraph + double buffer + staleness | VM-010 | dfaa62c, e47b057 | no `kSceneVersion`, layout asserts only in the clang test TU → VM-037; `MapElement` has no `last_update_sec` (HD-map pop-not-fade) → VM-036; single-threaded contract (spec §4.1 amended) |
| 2 Theme system + golden harness | VM-011 | 7eb1d25, fbd44c2, 6cfccba, de6f14e, 12b3ff3, e25865d, 196a561, 1506542, 2e7a363, e47b057 | `kFallbackTheme()` duplicates dark_adas.yaml with no drift test → VM-036 guards; fog scale is an empirical fit on `ibl.intensity` (spec §4.3 note); quality maps 2 of 5 knobs → VM-032; inline YAML in Step 4 is stale (note added) |
| 3 Animated theme transition | VM-014 | 83094df, e47b057 | `blend()` enumerates fields by hand (appended token blends to 0) → VM-036 guard; no ROS param picks the initial theme (always dark_adas at launch) → VM-044 |
| 4 Ego robot (M02P→glTF, TF pose, speed) | VM-012 | e51bad6 (+d18d283 axis bake, 02939c3 flatten_z) | conversion RAN (`~/Downloads/M02P.glb`, 75.8 MB, 2026-08-20) but the asset is outside git and `default_params.yaml` carries a per-user absolute path → VM-044; no light_clay ego golden (spec §10 PROPOSED scoping); `Hud` never populated → VM-030 |
| 5 vcam tween extraction | VM-013 | 7d4eec9, e51bad6 | none; the "known limitation" (40 m ground) was resolved by Epic 2 Task 2 (d18d283) |
...
> `[review 2026-09-07]` The two YAML blocks in this step are the **as-planned** values and are stale: the lux rebalance (6cfccba: dark sun/ibl → 480000/256000, light → 25000/8750), the fog rounds (12b3ff3, e25865d, 196a561) and the ref-2 re-authoring (3b3ce2c) superseded them. **Values live in `cuda/src/libs/visual_renderer/assets/themes/{dark_adas,light_clay}.yaml`; read them there.** The key SET below is still authoritative.
