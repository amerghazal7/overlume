...
**Epic 1: CLOSED at `bd5e11e` (gate round 1 applied), review-approved.** Every step
below is ticked because its deliverable is in a commit; FAIL-then-PASS ordering
inside a task is not separately verifiable (each task landed as one commit).

| Task | Backlog | Commits | Residual gaps (tracked where) |
|---|---|---|---|
| 1 SceneGraph + double buffer + staleness | VM-010 | 9cb0b2d, bd5e11e | no `kSceneVersion`, layout asserts only in the clang test TU → VM-037; `MapElement` has no `last_update_sec` (HD-map pop-not-fade) → VM-036; single-threaded contract (spec §4.1 amended) |
| 2 Theme system + golden harness | VM-011 | 152a8b3, 548efc0, d594fad, cd6d53e, db79d47, 65174b1, f01a2c9, 6285eaf, def81c0, bd5e11e | `kFallbackTheme()` duplicates dark_adas.yaml with no drift test → VM-036 guards; fog scale is an empirical fit on `ibl.intensity` (spec §4.3 note); quality maps 2 of 5 knobs → VM-032; inline YAML in Step 4 is stale (note added) |
| 3 Animated theme transition | VM-014 | 3651540, bd5e11e | `blend()` enumerates fields by hand (appended token blends to 0) → VM-036 guard; no ROS param picks the initial theme (always dark_adas at launch) → VM-044 |
| 4 Ego robot (M02P→glTF, TF pose, speed) | VM-012 | 7b561a0 (+49f7992 axis bake, 93f62f8 flatten_z) | conversion RAN (`~/Downloads/M02P.glb`, 75.8 MB, 2026-08-20) but the asset is outside git and `default_params.yaml` carries a per-user absolute path → VM-044; no light_clay ego golden (spec §10 PROPOSED scoping); `Hud` never populated → VM-030 |
| 5 vcam tween extraction | VM-013 | 496365b, 7b561a0 | none; the "known limitation" (40 m ground) was resolved by Epic 2 Task 2 (49f7992) |
...
**Prerequisite:** Epic 0 done and review-approved (commits `fac1535..5b479bd`). `FILAMENT_VERSION = 1.56.5` (pinned, see `cuda/src/libs/visual_renderer/README.md`). Task 6 (on-robot GPU budget) numbers are **pending** — see "Conservative perf assumptions" below.

**Goal:** Replace Epic 0's throwaway hello-frame scene (unlit cube+grid) with the real Epic-1 baseline: a `SceneGraph` domain model with double-buffered freeze-frame semantics and per-entity staleness fade, a data-driven theme system with both shipped themes on a genuinely **lit** clay pipeline (sun + IBL, not Epic 0's unlit deviation), an animated `set_theme` transition, and the ego robot rendered from TF with a smoothed finite-differenced speed. `set_scene`/`set_theme`/the `SceneGraph` POD layout are **stable and additive-only** after this one (`[superseded 2026-09-07 by ADR-0004]`: append fields/enum values/entry points and bump `kSceneVersion`; never rename, reorder or remove) (Epic 2 populates the categories Epic 1 leaves empty).
...
Epic 0 Task 6 (on-robot GPU budget measurement) has not run yet. Per project direction: default quality preset = **medium**, target resolution 1280×720@30. Nothing in this epic should be tuned against unmeasured numbers — goldens render at a fixed 320×240 purely for CI speed/determinism, not as a statement about the shipped default. **Stated precisely, not aspirationally: `RenderConfig::quality` is consumed by the renderer for exactly two toggles as of this epic — SSAO on/off+resolution and FXAA-vs-TAA (Task 2 Step 7a) — and nothing else.** Shadow-map resolution, render-resolution scaling, and the shadow-enable knob (spec §8's `low` tier is "FXAA only, **no shadows**" — a third preset-driven toggle, not just a resolution number) are still unmapped; that split is VM-032's job (Epic 3, "quality presets end to end") to finish. This isn't a scope creep: goldens are committed in this epic and freeze whatever the post chain does, so SSAO/AA had to be decided now (Task 2 Step 7a) the same way the ego's shadow-cast flags did — shadow *resolution*, shadow *enable/disable*, and render *resolution*, unlike SSAO/AA, don't change what a fixed-320×240, no-shadow-casting-object-yet golden looks like, so those three stay deferred without forcing an early decision. Goldens in this epic are captured at `quality=1` (medium: FXAA + SSAO half-res), matching the stated shipped default above. Do not gold-plate perf work here; Epic 5 (VM-040/041) owns hysteresis/auto-drop/benchmarking.

---

## Interfaces frozen this epic

`[superseded 2026-09-07 by ADR-0004 — additive-only, versioned]` Everything below is **stable** once Epic 1's review gate passes; Epic 2+ may only *add* new optional fields/entry points, never change these signatures or reorder these struct members (a CI-checked POD-layout snapshot test, added in Task 1, catches accidental breakage).

### `include/visual_renderer/scene.h` (new; extends `api.h`, still POD-only)

Spec §4.1 listed 8 scene categories at the time (it now lists 9 — `PointCloud[]` is appended in Epic 3 for VM-035 and was NOT in `scene.h` before that; `[review 2026-09-07]`). All 8 are POD structs now; Epic 1 only ever populates `ego` (`object_count == 0`, `path_count == 0`, etc. for everything else) — Epic 2 fills the rest without touching this header again.
...
struct Hud {
    // [audit 2026-09-07] NOT populated in Epic 1: visualization_node never assigns scene.hud
    // (speed_mps/active_mode stay 0). VM-030 adds the two assignments next to the ~/ego_state publish.
    double speed_mps;
...
**Gate-review addition (2026-08-20, spec §9 minor):** `create_renderer()` silently substitutes the compiled-in `kFallbackTheme()` whenever the requested `theme_assets_dir`/`initial_theme` fails to load, with no way for the caller to know it happened and `WARN`. Additive entry point in `scene.h` (legal under the freeze — new entry points may be added, existing signatures/struct layouts may not change):
```cpp
// True iff the theme active right after create_renderer() was actually
// loaded from disk, false if it's the compiled-in fallback. Reflects only
// the INITIAL load at create_renderer() time, not any later set_theme()
// call (which already has its own bool return for the same purpose).
bool theme_assets_loaded(VisualRenderer*);
```
Implemented by storing a bool on `VisualRenderer` at `create_renderer()` time (`loaded.has_value()` from the existing `load_theme()` call, before the `kFallbackTheme()` substitution). Asserted in `test_theme.cpp`'s existing `ThemeLoad.MissingThemeDir_FallsBackToBuiltinTheme` test (now also asserts `theme_assets_loaded(r) == false`) plus a new `ThemeLoad.RealAssetsDir_ThemeAssetsLoadedIsTrue` (asserts `true` against the real shipped `assets/themes` dir). `visualization_node.cpp`'s `on_configure()` calls it once right after `create_renderer()` succeeds and `RCLCPP_WARN`s (naming the `theme_assets_dir` it tried, or `"<compiled-in default theme dir>"` when the node passed `nullptr`, which it does today — no node parameter exposes this path yet) when it returns `false`.
...
- [x] **Step 1: Extend the POD check.** `check_pod_header.sh` currently hardcodes `api.h`'s path; change it to `for hdr in "$(dirname "$0")/../include/visual_renderer/"*.h`. Run it now — passes trivially (scene.h doesn't exist yet, glob matches only `api.h`).
- [x] **Step 2: Write `scene.h`** verbatim per the frozen block above. Run `check_pod_header.sh` — must still PASS (every field is POD/fixed-width/raw-pointer; this is the actual enforcement of the freeze, not just documentation).
- [x] **Step 3: Failing test — deep copy + freeze-frame.** In `test_scene_buffer.cpp`:
...
- [x] **Step 4: Implement `SceneBuffer`.** Deep-copy owner types (`std::vector`-backed, internal-only — never crosses the POD boundary):
...
- [x] **Step 5: Run — PASS.**
- [x] **Step 6: Failing test — staleness math**, table-driven:
...
- [x] **Step 7: Implement** (`age = now - last_update; return clamp(1 - (age - fade_start) / (timeout - fade_start), 0, 1)`). Run — PASS.
- [x] **Step 8: POD-layout snapshot test** (guards the freeze itself — catches an accidental member reorder/resize in review, not just a missing-field build error): `static_assert` a table of `offsetof`/`sizeof` for every struct in `scene.h`, in `test_scene_buffer.cpp` or a new `tests/test_scene_layout.cpp`:
...
```
  ...
  **Gate-review fix (MAJOR 2, 2026-08-20):** the version of this block that actually shipped through Task 2's review rounds narrowed "one line per struct/field that matters" down to `sizeof`-only for `TrackedObject`/`PathRibbon`/`MapElement`/`GroundGridLayer`/`AlertPolygon`/`GenericMarker`/`AlertChip`/`Hud` (no `offsetof` for any of their individual members) and left 9 of `SceneGraph`'s 15 members' offsets unchecked (`path_count`, `map_elements`, `map_element_count`, `grids`, `grid_count`, `alerts`, `alert_count`, `markers`, `marker_count`) — a same-size member reorder inside any of those structs (swapping two same-width fields, the exact kind of change a sizeof-only check can't see) would have passed CI silently. That silence matters more here than in a typical header: `micropilot_visualization_node` links a PREBUILT `libvisual_renderer.a` archive (Task 2's Files list), so a member reorder the header check missed would be a silent ABI break at the node's next rebuild, not a compile error at any call site. Fixed by adding an `offsetof` assert for EVERY member of every struct listed above (not just the ones judged "load-bearing"), plus — since `RenderConfig` (`api.h`) had no layout snapshot at all — `sizeof`/`offsetof` for its members too, in the same test file. All numbers below were read off this toolchain's actual compiler output (a tiny standalone probe against `include/`, run once), never hand-guessed:
```cpp
static_assert(offsetof(mpviz::TrackedObject, predicted_path) == 88, "...");
static_assert(offsetof(mpviz::SceneGraph, grid_count) == 112, "...");
static_assert(offsetof(mpviz::RenderConfig, theme_assets_dir) == 16, "...");
// ... full tables (every member, every struct in scene.h, plus RenderConfig
// in api.h) live in test_scene_buffer.cpp.
```
- [x] **Step 9: Build + test.**
```bash
...
```
- [x] **Step 10: Commit** `feat(visual): SceneGraph POD model + double-buffered scene + staleness fade (VM-010)`.
...
- [x] **Step 3: Failing test.** `test_theme.cpp`, first case just proves the material loads and differs visibly from flat unlit output under two different light directions (proves it's actually *lit*, not a copy of the unlit bug):
```cpp
...
> `[review 2026-09-07]` The two YAML blocks in this step are the **as-planned** values and are stale: the lux rebalance (d594fad: dark sun/ibl → 480000/256000, light → 25000/8750), the fog rounds (db79d47, 65174b1, f01a2c9) and the ref-2 re-authoring (ffa943a) superseded them. **Values live in `cuda/src/libs/visual_renderer/assets/themes/{dark_adas,light_clay}.yaml`; read them there.** The key SET below is still authoritative.

- [x] **Step 4:** Write both YAML files. Full token set from spec §4.3 (palette, material, emissive, grid, hud, sun, ibl) — every field below is consumed generically by `theme.cpp`, no per-theme branch anywhere in the renderer:
```yaml
# dark_adas.yaml
name: dark_adas
palette:
...
> **`[superseded by Epic 2 Task 1 Step 0.2 — do not do this]`** `libvisual_renderer.a` now absorbs yaml-cpp's objects and renames every `YAML::` symbol (`scripts/merge_yamlcpp.sh`, POST_BUILD), so nothing node-side resolves those names. Adding a second yaml-cpp archive to the gcc node link line reintroduces a silent wrong-data hazard (the node's `Profile.CoexistsWithTheRendererLibrarysOwnYamlCpp` test exists because an earlier variant "linked fine and still returned the wrong data"). Kept for history only.
...
- [x] **Step 5a: Give test binaries a PNG decoder, and give every test/library `kThemeDir`/`DEFAULT_THEME_ASSETS_DIR` a real, resolvable path.** Two separate gaps, fixed together because both land in the same `CMakeLists.txt` edit:
...
- [x] **Step 6:** Run test_theme.cpp's material test again — still needs `renderer.cpp` wiring (next).

### 2d. Wire it into `renderer.cpp`

- [x] **Step 7:** Replace the Epic 0 scene construction:
...
- [x] **Step 7c: Failing test for the Step 7b fallback**, in `test_theme.cpp`:
```cpp
...
- [x] **Step 7d: Run — PASS** (after Step 7b's fallback is implemented).
...
- [x] **Step 10:** `tests/test_theme.cpp` golden cases (`#include "test_paths.hpp"` for `kThemeDir`; golden paths use the `MPVIZ_TEST_DATA_DIR`-prefix convention from Step 5a so they resolve regardless of `gtest_discover_tests`'s build-dir working directory):
```cpp
...
- [x] **Step 11:** Generate the initial goldens deliberately (not silently from a passing test): run the same binary with an env var/flag that skips the comparison and just writes the PNG (`golden.cpp`'s `render_and_compare` already writes `out_png_path` unconditionally — copy that file to `tests/goldens/...png` after a **human looks at it**, matching the reference images' clay/matte look). `tests/golden.py` is the developer-facing wrapper for this (`python3 tests/golden.py --show /tmp/empty_world_dark_adas_actual.png` opens/prints it for a visual sanity check before committing — this project's stated working style is "user judges by visuals"). Commit the two PNGs.
- [x] **Step 12:** Run again — PASS (SSIM ≈ 1.0 against the just-committed goldens).
- [x] **Step 13: Build + full test run.**
```bash
cd cuda/src/libs/visual_renderer
cmake --toolchain cmake/toolchain-clang-libcxx.cmake -B build -S .
cmake --build build && ctest --test-dir build --output-on-failure
```
- [x] **Step 14: Commit** `feat(visual): real lit clay pipeline + theme system + golden-image harness (VM-011)`.
...
- [x] **Step 1: Oklab helpers.** `theme_transition.cpp` needs sRGB↔Oklab; small closed-form public-domain conversion (Björn Ottosson's reference formulas — cube-root + two 3×3 matrices each direction, ~15 lines per direction, no dependency):
```cpp
...
```
- [x] **Step 2: Failing test — mid-transition blend is perceptually plausible, not just component-lerped:**
```cpp
...
```
  Run — FAIL.
- [x] **Step 3: Implement** `blend(Theme, Theme, float t)`: `w = smoothstep(t)`; every `float3` palette/sun/hud color field → `blend_color`; every scalar (roughness, metallic, ribbon_strength, intensities, fade distances) → linear lerp by `w`. One function, applied uniformly — no per-field special-casing beyond "is this a color or a scalar," which keeps the "no per-theme code branches" property from Task 2 intact (this is "no per-*field* branches" too, just a type dispatch).
  ...
  **Deviation from this step's literal text (gate-review fix, MAJOR 1, 2026-08-20):** "every scalar ... → linear lerp by `w`" is also not true for `sun.intensity`/`ibl.intensity` — these two get a GEOMETRIC (log-space) lerp instead: `out = a * pow(b/a, w)`, guarding `a<=0`/`b<=0` by falling back to the linear lerp (never hit by either shipped theme — both are positive physical lux). Root cause: illumination × albedo is a product, and `dark_adas`→`light_clay` moves `sun.intensity` 480000→25000 lux (~19.2x) and `ibl.intensity` 256000→8750 lux (~29.3x) while the palette's albedo colors brighten at the same time — a plain linear lerp of the illumination side blended against a simultaneously-brightening albedo side overshoots BOTH endpoints around `w`~0.5–0.65 (measured on the pre-fix code: frame-mean luminance 43 at `t=0` → peaks ~205 around `t=0.6` → settles at 175 at `t=0.8`, i.e. a "dim to night" toggle visibly flashes brighter than day mid-transition, not just brighter than either endpoint alone). A geometric lerp is monotonic between its two endpoints by construction, so it cannot overshoot either one no matter how far apart they are. Regression test: `ThemeTransition.MidTransition_LuminanceDoesNotOvershootEndpoints` (`test_theme_transition.cpp`) renders `t=0.5`/`t=0.6` of the transition and asserts frame-mean luminance stays within `[min(endpoint means)-3, max(endpoint means)+3]` — written first against the linear lerp (failed: `t=0.6` mean measured 190.6, above the endpoint-max+3 bound of ~178.1) and passes after the geometric-lerp fix, on this build, with no measured overshoot at either sampled `t`. Colors stay Oklab (`blend_color`, unchanged); every other scalar (roughness, metallic, ribbon_strength, hud scale, fog density) stays a plain linear lerp — only these two intensity scalars get the geometric treatment, because they're the only ones multiplying against a simultaneously-animating albedo.

  One committed golden moved as a direct, expected consequence: `tests/goldens/transition_t0_4.png` (the mid-transition, `t=0.4s`/`w=0.5` frame — genuinely darker under the corrected math, since geometric-mean intensity at `w=0.5` is `sqrt(480000*25000)≈109545`/`sqrt(256000*8750)≈47329` lux, well below the old linear-lerp midpoints of 252500/132375 lux). `transition_t0.png`/`transition_t0_8.png` (the `t=0`/`t=0.8` endpoint/settled goldens) are unaffected by construction (`pow(x,0)==1` and `pow(x,1)==x/a*a==b` exactly) and were verified unchanged by re-running `ThemeTransition.DeterministicClock_MatchesTargetAtDuration`'s own SSIM compare against the committed files, not regenerated.
- [x] **Step 4: Run — PASS.**
- [x] **Step 5: Failing test — the transition state machine itself**, deterministic clock (this is the "deterministic-clock goldens" AC). `#include "test_paths.hpp"` for `kThemeDir`; golden paths use the same `MPVIZ_TEST_DATA_DIR`-prefix convention as Task 2 Step 10:
```cpp
...
- [x] **Step 6: Implement `set_theme`** in `renderer.cpp`: stores `{from: current_blended_theme(), to: load_theme(theme_name), start_sec: at_sec, duration_sec: transition_sec > 0 ? transition_sec : 0.8}` on the `VisualRenderer`. `render_frame` (or a new internal `apply_current_theme()` called at its top, before touching materials) computes `t = clamp((active_scene.sim_time_sec - start_sec) / duration_sec, 0, 1)`, calls `blend(from, to, t)`, and pushes the result's tokens into the existing material instances/sun/IBL/fog **every call** (cheap — `setParameter` calls, no reloads) — once `t >= 1.0` the transition struct is cleared (idempotent no-op blend after that, avoids recomputing forever). "No frame drop > 1 during switch" (AC) falls out for free: nothing here allocates, blocks, or reloads assets mid-transition, it's pure arithmetic + existing `setParameter` calls that already run every frame.
- [x] **Step 7: Generate + commit the 3 transition goldens** (same human-look-then-commit flow as Task 2 Step 11).
- [x] **Step 8: Run — PASS.**
...
- [x] **Step 9: Node wiring.** `visualization_node`: maintain `double sim_clock_sec_` (monotonic, incremented by the timer's own period each tick — `33ms` — rather than reading wall-clock, so the deterministic-clock contract holds all the way to the node too).
  ...
  **SIM-CLOCK DECISION (orchestrator, 2026-08-20 — documented here, no code changed):** `sim_clock_sec_ += kTimerPeriodSec` (fixed-step, incremented by the timer's OWN nominal period regardless of how long the tick actually took to run) is this step's deliberate deterministic-clock design, not an oversight — it's what makes `set_scene`/`render_frame`/goldens reproducible at all (this doc's own repeated "never wall-clock" framing). The trade-off that buys: under a sustained SLOW timer (a tick that takes meaningfully longer than `kTimerPeriodSec` to actually run), `sim_clock_sec_` still advances at the nominal rate while wall-clock time elapses faster per tick, so every renderer-side duration keyed off it stretches proportionally — an authored 0.8s `set_theme` transition plays out over roughly 1.2s of wall-clock time if the timer is only managing ~20fps instead of the nominal 30fps (`0.8 * 30/20`), not because the transition math changed, but because "0.8s" here means 0.8 sim-seconds, and sim-seconds only equal wall-seconds when the timer holds its nominal rate. Measured drift on this dev box today is ~0% (Task 4 Step 11's re-run and the WS E2E test both hit nominal cadence), so this is a recorded trade-off, not a live defect. The mitigation already scoped for it is Epic 5's auto-drop (VM-040/041, spec §8's hysteresis/quality-drop-under-load mechanism) — its whole job is keeping the timer at nominal rate on constrained hardware in the first place, which is what keeps this clock's sim-second/wall-second equivalence holding outside this dev box too.
...
```
- [x] **Step 10: WS bridge command.** `tools/vcam_ws_bridge.py`: `{"cmd": "set_theme", "theme": "dark_adas"|"light_clay"}` → publish `std_msgs/String` to `/visualization_node/set_theme` (node-private, mode-3-only concept — no mux needed, harmless if published while mode 1/2 is active, matches the "ingest continues regardless of mode" philosophy).
- [x] **Step 11: GUI toggle.** `tools/vcam_gui.py`: a day/night button next to the existing mode button (same `Gtk.Button` pattern as `_mode_btn`/`_on_mode_toggle`), sending the WS command above; label reflects last-known theme the way `_mode_btn`'s label already tracks `render_mode` from telemetry (Epic 3's `~/diagnostics`, VM-034, is the eventual place a theme-name echo would live end-to-end — for now the GUI just optimistically flips its own label on click, same as how it doesn't wait for confirmation on preset buttons today).
...
```
- [x] **Step 13: Full rebuild + existing-suite regression check** (Global Constraint: everything upstream stays green):
```bash
...
```
- [x] **Step 14: Commit** `feat(visual): animated set_theme transition (Oklab, smoothstep, 0.8s default) + GUI/WS toggle (VM-014)`.
...
- [x] **Step 1:** `obj2gltf_m02p.py` — thin wrapper, not a bespoke parser:
```python
#!/usr/bin/env python3
...
- [x] **Step 2:** Run for real against the user's file (manual, one-time, not part of any test suite — the output is a large binary that stays out of git, same as the source OBJ):
```bash
...
- [x] **Step 3: Verify gltfio ships in the pinned SDK** before writing any code against it:
```bash
...
- [x] **Step 7: Failing test.** `test_tf_adapter.py` — recorded fixture: a short sequence of `map→base_link` transforms at known timestamps/positions (straight-line motion at a known constant speed, e.g. 2.0 m/s), fed through the adapter, asserting the finite-differenced+smoothed speed converges to 2.0 m/s within a tolerance after the smoothing filter's settling time (and that a single noisy outlier sample doesn't spike the reported speed — that's what the smoothing is *for*):
```python
...
- [x] **Step 11: Full rebuild + regression.**
```bash
cd cuda/src/libs/visual_renderer && cmake --build build && ctest --test-dir build --output-on-failure
cd cuda/scripts/ros_apps_build && ./colcon_build.sh
...
- [x] **Step 12: Commit** `feat(visual): ego robot from M02P glTF, TF-driven pose + smoothed speed, clay-box fallback (VM-012)`.

  **Status (corrected 2026-08-20 — this block was stale):** DONE — committed as `7b561a0` (`feat(visual): ego robot from M02P glTF, TF-driven pose + smoothed speed, clay-box fallback (VM-012)`), and that commit also folded in the live additions from the 2026-08-19 validation session (Task 5's ego-anchored-camera composition and the GUI 3-way view cycle documented in that task's scope-addition blocks above) rather than leaving them for Task 5's own commit. **Step 2: DONE (2026-08-20, manual; corrected 2026-09-07 review).** `/home/ag7/Downloads/M02P.glb` (75.8 MB) was produced from the user's OBJ and is what `config/default_params.yaml` points at; the asset is intentionally outside git. **Deviation landed later (Epic 2, 49f7992):** `obj2gltf_m02p.py` now bakes the OBJ→rig axis rotation (verified live 2026-08-20; documented only in the script until now). Remaining debt: the per-user absolute path and the un-installed asset → backlog VM-044.
...
**Scope addition, other half of the same 2026-08-19 user directive:** `tools/vcam_gui.py`'s existing 2-way render-mode toggle button became a 3-way cycle — `bowl -> pointcloud -> visual` (`{1: 2, 2: 3, 3: 1}` next-mode map in `_on_mode_toggle`) — now that mode 3 (visual) is a real, live-validated render target rather than a stub. Same pattern as the button's pre-existing behavior: `_on_mode_toggle` sends `set_render_mode` over the WS bridge and flips `self._render_mode`/the button's label (`"view: bowl"`/`"view: pointcloud"`/`"view: visual"`) **optimistically**, before any telemetry confirms the switch — it doesn't wait for a round trip, same as the preset buttons never have; the label is reconciled the next time telemetry's `render_mode` field arrives (`_on_telemetry`), the same self-correcting-on-next-tick behavior the day/night theme toggle below already documents for itself. No new WS command needed — `set_render_mode` already accepted `"visual"` as a value from earlier Epic-0 wiring, this only changes which modes the button's own click cycles through.

**Known limitation (2026-08-19 user directive, live validation session — deferred, not fixed here):** with the camera now ego-anchored, the ego can drive off the edge of the static ground/grid. `build_ground_plane`/`build_grid_lines` build a fixed 40m extent centered on the *world origin* once, and the grid's line alpha fades by each line's radial distance from that same origin — neither is a function of the ego's live position. Once the ego travels beyond roughly 20m from the origin, the follow-cam frames it against bare void: the ground it's actually standing on has already faded out or ended. This is Epic-2-adjacent (real map/OGM ground content) rather than an Epic 1 defect, so it is being recorded, not fixed, pending a user decision between two candidates: (a) re-centre the ground/grid under the ego every frame, quantizing the re-centre step to the grid spacing so the lines don't visibly swim as the ego moves, or (b) leave the static origin-centered ground as today's placeholder and wait for Epic 2 to supply real map/OGM-derived ground geometry that has no origin-centered extent to outrun. **Resolved in Epic 2 Task 2 (VM-024, 49f7992) — `[review 2026-09-07]`:** candidate (a) shipped: the ground/grid patch follows `ego.position` in XY, quantized to the 2 m grid pitch, snapping to the origin when `ego.valid == 0`; goldens `map_ego_offset_{dark_adas,light_clay}.png`. Nothing open here.
...
- [x] **Step 1: Confirm the safety net exists and is green before touching anything.**
```bash
...
- [x] **Step 3: `visualization_node.cpp` shrinks.** `on_configure`: `vcam_ = std::make_unique<Vcam>(this, seed_pose);` replaces the inline preset-table setup. `timer_callback`: `vcam_->advance_tween(); pose_ = vcam_->pose();` replaces the direct member access. `on_cleanup`/`on_shutdown`: `vcam_.reset();` replaces the individual `set_vcam_srv_.reset(); set_look_sub_.reset();`.
- [x] **Step 4: Rebuild + re-run the exact same contract test — must still PASS, unchanged assertions, unchanged tolerances.**
```bash
...
```
- [x] **Step 5: Commit** `refactor(visual): extract vcam preset/tween into src/vcam.cpp (VM-013)` — no behavior change, so the commit body should say exactly that and point at Step 4's unchanged-test evidence.
...
- Spec §4.1 (all 8 SceneGraph categories of the time present as POD — the 9th, `PointCloud[]`, is Epic 3's; Ego populated), §4.2 (lit pipeline replacing the Epic 0 unlit deviation — sun + IBL both visibly doing something, not dead code), §4.3 (both themes, `set_theme` animated per the 0.8s/Oklab/smoothstep contract), §4.4 (ego glTF + non-fatal fallback), §9 (asset-load-failure and no-TF-yet paths both non-fatal), §10 (golden-image harness exists and is GPU-skip-clean).
...
- No task exceeded its stated scope (e.g., Task 5 stayed a pure refactor; Task 2's IBL simplification is documented, not silently narrower than spec; Task 2's SSAO/FXAA-vs-TAA mapping is wired per `RenderConfig::quality` and baked into this epic's goldens at `quality=1`, while shadow-map-resolution/render-resolution/shadow-enable stay an explicit, named deferral to Epic 3's VM-032, not a silent scope drop).

**Epic 1 results (filled 2026-08-20):** golden SSIM thresholds actually used = **0.98**, uniformly, for every committed golden in this epic (`EgoGolden.ClayBoxFallback_DarkAdas`, `ThemeGolden.EmptyWorld_{DarkAdas,LightClay}`, `ThemeTransition.DeterministicClock_MatchesTargetAtDuration`'s three `transition_t0{,_4,_8}.png` goldens) — block-SSIM per `golden.cpp`'s `render_and_compare()`. gltfio-in-prebuilt-SDK outcome (Task 4 Step 3) = ships in the pinned Filament 1.56.5 SDK, split across `libgltfio.a` + `libgltfio_core.a` (+ `libuberarchive.a`), all swept onto the link line by `GetFilament.cmake`'s existing `file(GLOB _filament_static_libs "${FILAMENT_ROOT}/lib/x86_64/*.a")` — no separate `Filament::gltfio` CMake target or node `CMakeLists.txt` change needed. Perf numbers incidentally observed (informational only — Task 6 is still the gate) = 1280×720, `quality=1`, ~30Hz steady (mean tick 33.0ms) on dev RTX 3090 under sim load; E2E max inter-frame gap observed 19-57ms across theme-switch/mode-switch tests.
