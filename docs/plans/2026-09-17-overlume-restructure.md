...
| Task | Title | Status |
|---|---|---|
| 0 | LFS + deletion of legacy code and agent-config sprawl | ☑ 2026-09-17 — 2c54d12 (deletions, 94 files), 65a6a3f (.gitattributes), LFS pointer commit (73 binaries, `--no-rewrite`, bytes verified identical). Gate PASS with unchanged counts. Note: `git lfs install` collides with graphify's `post-checkout` hook — the LFS lines were appended to the existing hooks by hand; contributors get this via `git lfs install` on a fresh clone where no conflict exists. |
...
| 6 | Open-source scaffolding: LICENSE/NOTICE/SPDX, CONTRIBUTING, CoC, SECURITY, CHANGELOG, `.github` CI + release | ☑ 2026-09-17 — 7db1ebb (scaffolding + workflows) and 623d088 (one repo-wide clang-format, clang-format 23.1.1 pinned via PyPI, SPDX on 242 files, ctest `cpu`/`gpu` labels; listed in `.git-blame-ignore-revs`). Repo made public and Pages (source: GitHub Actions) + private vulnerability reporting enabled 2026-09-17; site <https://amerghazal7.github.io/overlume/>. **Verified 2026-09-17 on the first push:** all four workflows green (the hosted build first failed on a real defect — `test_gltf_normals` compiled against a system yaml-cpp header locally — fixed in 3e213f3), `v0.1.0` released, Pages live; the hosted build depends on apt.llvm.org's jammy pool keeping the pinned LLVM 18.1.8 `.deb`s. Gate round 1 (6b) caught a `nogpu` label that ctest's unanchored `-L` regex would have matched against `gpu`; renamed `cpu`. |
