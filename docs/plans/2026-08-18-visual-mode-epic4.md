...
| 2 `bake_environment.py`: OSM footprints → chunked, georeferenced glTF + index + verification overlay | VM-051 | **Done 2026-09-10** | All 7 steps landed at `ec661a6`; round-1 review fixes (global `--selfcheck` network guard, `load_ego_track_xy` column pin, +x mode) at `81c654f`; round-2 minors (index-row center/radius asserts, nonzero-heading pin probe, malformed-ring skip) applied post-gate. Two deviations from the Files/Steps text: (a) Step 5's three height-fallback cases use synthetic in-memory footprints rather than extending the committed Overpass fixture — no way to carry height/`building:levels` tags into the real fixture without fabricating tags into a real-provenance source, rejected per the note already recorded in `assets/models/environment/ATTRIBUTION.md`; (b) Step 4's ego-track polyline reuses Task 1's existing `test/fixtures/geo_anchor_samples_0.csv` via an optional `--ego-track` flag instead of committing a second track fixture. |
...
**Prerequisite:** Epic 3 is CLOSED at `af628df` (gate PASSED-with-findings 2026-09-09, none blocking). No Epic 3 finding touches this epic's files. The current stack recording is `stack_v2_fixtures_2026-09-09` (validate default) and `stack_v2_full_sensors_2026-09-09` (camera+lidar superset) — both used as this epic's fixture data below; `epic2_fixtures_full` is NOT used (it predates the confirmed 50 Hz GPS measurement and was never independently re-measured for it).
...
- [x] **Step 3: Quadtree chunking (~256 m cells) + index.** **Done 2026-09-10.** far->2 chunks (want 2), near->1 chunk (want 1) -> OK. Bucket extruded footprints by their centroid's `(floor(x/256), floor(y/256))` cell; each non-empty cell becomes one `chunk_<i>_<j>.glb` (all its footprints as one scene, `trimesh.util.concatenate` or a scene with multiple geometries — either is fine since Task 3 remaps every primitive's material regardless of source grouping, Decision 4). Write `index.yaml` with each chunk's map-frame center + bounding-sphere radius. Failing test (`--selfcheck`): two synthetic footprints placed > 256 m apart bake into two distinct chunk files; two placed in the same cell bake into one. Run — FAIL then PASS.

- [x] **Step 4: Verification overlay image.** **Done 2026-09-10.** Overlay written and well-formed (non-degenerate dimensions); `load_ego_track_xy()`'s column contract independently pinned against Task 1's exact CSV format. Rasterize every footprint's map-frame XY outline plus the recorded ego track (from a small extracted `(x,y)` polyline fixture, same committed-slice convention as Task 1 Step 1) onto one image, `verification_overlay.png` (or `.ppm` fallback). Failing test (`--selfcheck`): the file is written and non-empty, and its pixel dimensions are non-degenerate (> 1×1) — this is a human-sanity-check artifact per the AC ("overlay image sanity-approved"), so the automated test only proves the artifact exists and is well-formed, not that it "looks right"; that judgment is the AC's own human step, named as such rather than faked with a pixel-diff.
  Run — FAIL then PASS.

- [x] **Step 5: `--selfcheck` end-to-end, height-fallback cases named explicitly.** **Done 2026-09-10.** `--selfcheck` 7/7 PASS; all three height cases (has_height/has_levels/has_neither) match Decision 9's fallback chain exactly. Extend the committed fixture with three footprints: one carrying an OSM `height` tag, one carrying only `building:levels`, one carrying neither — assert each bakes to the height Decision 9's fallback chain predicts (parsed back from the `.glb`'s own bounding-box Z extent). Run full `--selfcheck` — PASS, all cases.

- [x] **Step 6: Commit** `feat(visual): bake_environment.py — OSM footprints -> chunked map-frame glTF + index (VM-051)`. **Done 2026-09-10.** Landed at `ec661a6`.
...
- [x] **Step 5: Perf check.** **Done 2026-09-10 — delta 0.02–0.38 ms across all recorded runs on this dev box (two batches: implementer 0.081/0.062/0.058/0.381, reviewer 0.096/0.055/0.018), dev-box proxy, PASS against the < 2 ms bar; the results block below states the same figure.** Measure `render_ms` (VM-034's existing instrumentation, unchanged) with the committed test town loaded vs. not loaded, at the shipped default quality preset, on this dev box — same proxy-only status every number in `budget_probe.md` carries (Decision 11: the on-robot rerun is Epic 5's, not this epic's). Record the delta in the results block below against the AC's `< 2 ms` bar. If the delta exceeds it on this fixture town's chunk count, that is a finding for the review gate, not silently absorbed.
  Run — record result.

- [x] **Step 6: Commit** `feat(visual): runtime chunk load + distance culling behind EnvironmentSource, palette.building (VM-052)`. **Done 2026-09-10** (`88dbc24`); review round 1 findings fixed in a follow-up commit the same day — see the deviations below.

**Task 3 deviations (2026-09-10, recorded at close, not just in git):**

1. **Node package builds and tests in this worktree via separate colcon bases (review round 2 disproved the earlier “not runnable” claim).** `colcon --log-base <scratch> build --packages-select micropilot_visualization_node --build-base <scratch> --install-base <scratch>`, sourcing the MAIN checkout's install space read-only for `micropilot_rendering_node`, finishes clean and the package's 221 tests pass. The residual gap: a build against a worktree-local `micropilot_rendering` (CUDA) install was not attempted — only the shared install space under the main checkout is off-limits to writes. The merge step re-verifies on main as usual.
...
3. **Fixture provenance.** `tests/fixtures/environment_test_town_0/` was baked with vm051's `bake_environment.py` at commit `ec661a6` and re-verified byte-identical against vm051's current HEAD script.
...
- `render_ms` delta measured with the test town loaded vs. not, at the shipped default preset: delta 0.02–0.38 ms across all recorded runs on this dev box (two batches: implementer 0.081/0.062/0.058/0.381, reviewer 0.096/0.055/0.018), dev-box proxy, AC bar < 2 ms — PASS
