    ...
    py::class_<Reprojector>(m, "Reprojector")
        ...
        .def("render_bowl",
             [](Reprojector& r, py::dict vcam, float R0, float k, float Rmax,
                float feather_margin, bool fill_blind_zone, bool exposure_match) {
                 CameraParams v = to_cam(vcam);
                 BowlParams b{R0, k, Rmax, feather_margin, fill_blind_zone, exposure_match};
        ...
        .def("render_hybrid",
             ...
             py::arg("fill_blind_zone") = false, py::arg("exposure_match") = false);
