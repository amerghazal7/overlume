...
// ---------------------------------------------------------------------------
static std::array<float, 3> bench_cross3(std::array<float, 3> a,
                                          std::array<float, 3> b)
{
...
static CameraParams make_bench_ring_cam(float theta_rad)
{
    ...
    CameraParams cam{};
    cam.K[0] = fx; cam.K[2] = cx; cam.K[4] = fy; cam.K[5] = cy; cam.K[8] = 1.f;
    ...
    cam.t[0] = radius * ct; cam.t[1] = radius * st; cam.t[2] = mount_h;
...
TEST(Reprojector, BowlRenderFloor720p)
{
    ...
    vcam.K[0] = vfx; vcam.K[2] = OUT_W / 2.f;
    vcam.K[4] = vfx; vcam.K[5] = OUT_H / 2.f; vcam.K[8] = 1.f;
    vcam.R[0] = 1.f; vcam.R[4] = 1.f; vcam.R[8] = 1.f;
    vcam.t[0] = 0.f; vcam.t[1] = 0.f; vcam.t[2] = 5.f;
    vcam.width = OUT_W; vcam.height = OUT_H;
