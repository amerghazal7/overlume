...
/** Cap for the per-pixel per-camera scratch used by exposure statistics. */
constexpr int kMaxCams = 12;
 ...
 *  d_gains (nullable): per-camera exposure gains multiplied into each sample.
 *  d_pair_stats (nullable): (ncam*ncam*3) floats — for each camera pair (i<j)
 *  covering a (subsampled) pixel, accumulates [sum lum_i, sum lum_j, count]
 *  at [(i*ncam+j)*3] via atomics; raw (pre-gain) luminances. */
 ...
 *  which launch_splat treats as "skip". d_gains (nullable): per-camera
 *  exposure gains — keep point colors consistent with the gain-matched bowl. */
