...
TEST(GenericMarkersGolden, EveryPrimitiveType_DarkAdas) {
    ...
    const std::string meshPath = std::string(MPVIZ_TEST_DATA_DIR) + "/tests/fixtures/test_cube.glb";
    mpviz::testing::GenericMarkerScene scene =
        mpviz::testing::make_all_primitive_markers(/*now=*/10.0, meshPath.c_str());
    mpviz::SceneGraph s{};
    s.sim_time_sec = 10.0;
    ...
    double ssim = mpviz::testing::render_and_compare(
        r, pose, MPVIZ_TEST_DATA_DIR "/tests/goldens/markers_parity_dark_adas.png",
...
TEST(GenericMarkers, PooledRenderablesNoPerFrameAllocation) {
    ...
    mpviz::CameraPose pose{{12, -14, 10}, {12, 3, 0}, 70.0};
    render_once(r, pose);
    const uint32_t afterFirst = mpviz::testing::generic_marker_alloc_count(r);
    EXPECT_GT(afterFirst, 0u) << "the first frame must actually allocate something";

    for (int i = 0; i < 30; ++i) {
        render_once(r, pose);
    }
    EXPECT_EQ(mpviz::testing::generic_marker_alloc_count(r), afterFirst)
...
TEST(GenericMarkers, UnknownOrUnsupportedPrimitiveIsSkippedAndCounted) {
    ...
    m.primitive = static_cast<mpviz::MarkerPrimitive>(200);  // outside the frozen 0..9 range
    m.position = {0, 0, 0.5};
    m.scale = {1, 1, 1};
    mpviz::SceneGraph s{};
    s.markers = &m;
    ...
    EXPECT_EQ(mpviz::testing::generic_marker_unknown_count(r), 1u);
    EXPECT_EQ(mpviz::testing::generic_marker_slot_count(r), 1u);
    mpviz::destroy_renderer(r);
}
...
TEST(GenericMarkers, MeshPathLoadFailureFallsBackToClayBoxWarnOnce) {
    ...
    EXPECT_TRUE(mpviz::testing::generic_marker_mesh_is_fallback(r, 0));
    mpviz::destroy_renderer(r);
}
...
TEST(GenericMarkers, ZeroAlphaColorUsesThemeNeutralDefault) {
    ...
    mpviz::GenericMarker m{};
    m.primitive = mpviz::MarkerPrimitive::CUBE;
    m.position = {0, 0, 0.5};
    m.scale = {1, 1, 1};
    ...
    const auto tint = mpviz::testing::generic_marker_tint(r, 0);
...
TEST(GenericMarkers, StaleMarkersFadeViaSharedStalenessAlpha) {
    ...
    mpviz::GenericMarker markers[2]{};
    markers[0].primitive = mpviz::MarkerPrimitive::CUBE;
    markers[0].position = {0, 0, 0.5};
    markers[0].scale = {1, 1, 1};
    markers[0].last_update_sec = 10.0;  // fresh at sim_time 10.0

    markers[1].primitive = mpviz::MarkerPrimitive::CUBE;
    ...
    const auto fresh = mpviz::testing::generic_marker_material_info(r, 0);
    EXPECT_FALSE(fresh.bound_to_translucent);
    EXPECT_FLOAT_EQ(fresh.alpha, 1.0f);

    const auto stale = mpviz::testing::generic_marker_material_info(r, 1);
