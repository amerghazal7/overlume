...
// two covering cameras' baked feather*alignment^2 weight and camera-slot
// index -- Decision 3's own construction rule: camera-slot assignment is
// uniform per TRIANGLE (all three vertices carry the identical (index_a,
// index_b) pair, weight 0 where a slot's camera doesn't cover that
// vertex), because CUSTOM0's index components are linearly interpolated
// by the rasterizer and an index that varies within one triangle would
// read as fractional garbage. This bake enforces that rule by never
// sharing a logical grid vertex across triangles -- each of a triangle's
// three corners gets its own duplicated BowlVertex record, its weight_a/b
// computed against that ONE triangle's own camera-pair decision (ponytail:
// bowl tessellation only needs to approximate the surface shape, per
// Decision resolution 2 -- the extra vertex-buffer memory this trades for
// is cheaper than detecting cross-triangle agreement first).
#pragma once
...
// bakes each vertex's up-to-2 covering cameras' weight (feather x
// alignment^2, matching reproject.cu's bowl_kernel exactly --
// bowl_projection::BorderFeather/CameraAlignment) + camera-slot index,
// enforcing the per-triangle-uniform-index construction rule above.
// `camera_count` <= kMaxBowlCameras; `extrinsics`/`intrinsics`/`cam_width`/
// `cam_height` are `camera_count`-entry arrays, same contract as
// BowlConfig. `feather_margin` is in source-image pixels (BowlConfig's own
// unit).
