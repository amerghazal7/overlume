...
mpviz::Vec3 WgsToMap(const mpviz::GeoAnchor& anchor, double lat_deg, double lon_deg, double alt_m)
{
    ...
    return mpviz::Vec3{east * s + north * c, -east * c + north * s, alt_m};
}

std::pair<double, double> MapToWgs(const mpviz::GeoAnchor& anchor, mpviz::Vec3 map_xy)
{
    const double h = anchor.heading_rad;
    const double s = std::sin(h), c = std::cos(h);
...
mpviz::GeoAnchor SolveAnchor(const std::vector<std::pair<double, double>>& fixes,
...
{
    anchor_ = mpviz::GeoAnchor{lat_deg, lon_deg, heading_deg * kDegToRad};
    overridden_ = true;
    solved_ = true;
}
