...
void TfAxesAdapter::fill(micropilot::visualization_app::SceneAssembly& out, double sim_time_sec)
{
    ...
    for (const auto& frame : frames)
    {
        ...
        for (const Axis& axis : kAxes)
        {
            ...
            point_storage_.push_back(mpviz::Vec3{origin.x(), origin.y(), origin.z()});
            point_storage_.push_back(mpviz::Vec3{tip.x(), tip.y(), tip.z()});

            mpviz::GenericMarker m{};
            m.primitive = mpviz::MarkerPrimitive::LINE_LIST;
