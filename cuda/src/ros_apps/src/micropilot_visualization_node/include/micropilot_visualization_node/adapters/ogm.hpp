 ...
 *  (visualization_node.cpp) walks subscriptions_for(row) and binds each
 ...
 *  every other adapter -- visualization_node.cpp checks stats().
...
{
public:
    OgmAdapter(const ProfileRow& row, const micropilot::visualization_app::FrameTransformer& tf);
