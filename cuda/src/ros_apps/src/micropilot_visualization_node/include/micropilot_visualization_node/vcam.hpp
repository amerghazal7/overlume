 ...
 *  from visualization_node.{hpp,cpp} (plan Task 5 / VM-013).
 *
 ...
 *  (ego_anchor.hpp) stays in visualization_node.cpp's timer_callback, at the
...
/// Extracted verbatim from VisualizationNode (plan Task 5 / VM-013) — the
...
public:
    ...
    Vcam(rclcpp_lifecycle::LifecycleNode* node, const mpviz::CameraPose& seed_pose);
    ...
    const mpviz::CameraPose& pose() const { return pose_; }
...
private:
    ...
    mpviz::CameraPose pose_{};  // current (possibly tweening) pose
