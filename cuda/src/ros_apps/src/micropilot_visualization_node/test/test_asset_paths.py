...
Run (ROS + this repo's ros_apps install sourced first, node built Release):
    source /opt/ros/humble/setup.bash
    source cuda/install/ros_apps/setup.bash
    python3 cuda/src/ros_apps/src/micropilot_visualization_node/test/test_asset_paths.py
...
PKG = "micropilot_visualization_node"
...
def _launch(extra_args: str, log_path: str) -> subprocess.Popen:
    cmd = (
        ...
        f"ros2 run {PKG} visualization_node --ros-args "
...
def main() -> int:
    ...
    if not os.path.isdir(INSTALL_DIR):
        print("SKIP: cuda/install/ros_apps not built -- run colcon build first.")
        return 0
    ...
    print("PASS: visualization_node asset-path resolution test passed.")
    return 0
