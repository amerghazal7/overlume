...
Run (ROS + this repo's ros_apps install sourced first):
    ...
    python3 cuda/src/ros_apps/src/micropilot_visualization_node/test/test_bowl_node_params.py
    ...
    try:
        if not _wait_running(viz_proc):
            with open(log_path) as f:
                tail = f.read()[-2000:]
            print(f"FAIL: visualization_node exited early (default params):\n{tail}",
                  file=sys.stderr)
            return 1
        ...
        if viz_proc.poll() is not None:
            with open(log_path) as f:
                tail = f.read()[-2000:]
            print(f"FAIL: visualization_node crashed after activate with default params "
        ...
        print("PASS: visualization_node bowl-param node-level regression test passed.")
        return 0
    finally:
        _kill(viz_proc)
        try:
