...
Run (ROS + this repo's ros_apps install sourced first, per plan Task 4 Step
11):
    source /opt/ros/humble/setup.bash
    source cuda/install/ros_apps/setup.bash
    PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest \
        cuda/src/ros_apps/src/micropilot_visualization_node/test/test_tf_adapter.py

Skips cleanly when this repo's ros_apps install isn't built yet.
"""
...
def test_tf_adapter_speed_converges():
    if not os.path.isdir(INSTALL_DIR):
        pytest.skip("ros_apps install/ not found -- build with colcon_build.sh first")
