...
// VisualizationNode/rclcpp harness in this suite" shape as
...
namespace
{
using micropilot::visualization_app::ColorizeFromCameras;
...
TEST(LidarColorize, PointInSingleCameraFovGetsThatCamerasColor)
{
    ...
    const std::vector<mpviz::Vec3> points{{0, 0, 5}};  // straight ahead, depth 5
...
TEST(LidarColorize, PointOutsideEveryCameraFovIsDropped)
{
    ...
    // own bounds check).
    const std::vector<mpviz::Vec3> points{{0, 0, 5}, {1000, 1000, 5}};
    const std::vector<const uint8_t*> bufs{rgb.data()};
...
TEST(LidarColorize, PointVisibleToTwoCamerasPicksFirstConfiguredMatch)
{
    ...
    const std::vector<mpviz::Vec3> points{{0, 0, 5}};
