...
mpviz_node::ProfileRow MakeRow(const std::string& color_mode = "auto", uint32_t max_points = 0,
                               uint32_t stride = 1)
{
...
const mpviz::PointCloud* OnlyCloud(const SceneAssembly& asm_)
{
...
TEST(PointCloudAdapter, AutoModePicksRgbWhenFieldPresent)
{
    ...
    TfFixture kTf;
    mpviz_node::PointCloudAdapter a(MakeRow("auto"), kTf.tf);
    a.ingest(msg, /*sim_time_sec=*/1.0);
    ASSERT_EQ(a.stats().msgs, 1u);

    SceneAssembly asm_;
    a.fill(asm_);
    const mpviz::PointCloud* pc = OnlyCloud(asm_);
...
TEST(PointCloudAdapter, IntensityRangeAutoRangesWhenRowLeavesItUnset)
{
    ...
    auto msg = BuildCloud(pts, "intensity");
    TfFixture kTf;
    mpviz_node::PointCloudAdapter a(MakeRow("intensity"), kTf.tf);
    a.ingest(msg, 1.0);
...
TEST(PointCloudAdapter, DecimationRespectsMaxPointsAndStride)
{
    ...
    auto msg = BuildCloud(pts, "rgb");
    TfFixture kTf;
    mpviz_node::PointCloudAdapter a(MakeRow("rgb", /*max_points=*/3, /*stride=*/2), kTf.tf);
    a.ingest(msg, 1.0);
...
TEST(PointCloudAdapter, MissingXyzFieldDropsWholeMessage)
{
    ...
    TfFixture kTf;
    mpviz_node::PointCloudAdapter a(MakeRow(), kTf.tf);
    a.ingest(msg, 1.0);
...
TEST(PointCloudAdapter, FlatModeBakesTheAlphaZeroSentinel)
{
    ...
    auto msg = BuildCloud(pts, "");
    TfFixture kTf;
    mpviz_node::PointCloudAdapter a(MakeRow("flat"), kTf.tf);
    a.ingest(msg, 1.0);
...
// vendored copy (MPVIZ_NODE_STB_DIR, CMakeLists.txt) -- NOT visual_
...
void RenderGoldenAndAssert(const std::string& tier_name, const std::string& extra_field_name)
{
    ...
    TfFixture kTf;
    mpviz_node::PointCloudAdapter adapter(MakeRow("auto"), kTf.tf);
    ...
    mpviz::FrameView view{frame.data(), kGoldenWidth, kGoldenHeight};
    mpviz::CameraPose pose{{0.0, -14.0, 12.0}, {0.0, 0.0, 0.0}, 60.0};
    ...
    const std::string golden_path =
        std::string(MPVIZ_NODE_FIXTURES_DIR) + "/point_cloud_" + tier_name + "_golden.png";
