...
Run (ROS + this repo's ros_apps install sourced first, from a box with a GPU
-- render_frame() needs one):
    source /opt/ros/humble/setup.bash
    source cuda/install/ros_apps/setup.bash
    python3 cuda/src/ros_apps/src/micropilot_visualization_node/test/test_mode_dispatch_pixels.py \\
...
# render_mode_ int constants -- visualization_node.hpp's kRenderModeBowl/
...
def main() -> int:
    ...
    vparams = _run(
        "ros2 pkg prefix micropilot_visualization_node"
    ).stdout.strip() + "/share/micropilot_visualization_node/config/default_params.yaml"
