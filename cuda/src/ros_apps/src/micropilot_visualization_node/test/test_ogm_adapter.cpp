...
const mpviz::GroundGridLayer* OnlyGrid(const SceneAssembly& asm_)
{
...
TEST(OgmAdapter, UnknownCellsBecomeTheSentinelNotTwoFiftyFive)
{
    ...
    EXPECT_EQ(g->cells[0], mpviz_node::kUnknownCell) << "-1 (unknown) must become the sentinel";
    EXPECT_EQ(g->cells[1], 0u);
    EXPECT_EQ(g->cells[2], 50u);
    EXPECT_EQ(g->cells[3], 100u);
    EXPECT_EQ(g->cells[4], mpviz_node::kUnknownCell)
...
{
    auto grid = mpviz_node::testing::load_occupancy_grid("ogm_synthetic.yaml");
    auto patch = mpviz_node::testing::load_occupancy_grid_update("ogm_update_synthetic.yaml");
    ASSERT_EQ(patch.x, 1);
    ASSERT_EQ(patch.y, 1);
    ...
    // full-grid ingest.
    EXPECT_EQ(g->cells[0], mpviz_node::kUnknownCell);
    EXPECT_EQ(g->cells[1], 0u);
    EXPECT_EQ(g->cells[2], 50u);
    EXPECT_EQ(g->cells[3], 100u);
    EXPECT_EQ(g->cells[4], mpviz_node::kUnknownCell);
    ...
    EXPECT_EQ(g->cells[7], mpviz_node::kUnknownCell) << "-1 in the PATCH must also convert to the sentinel";
...
TEST(OgmAdapter, RoleSelectsLayerKind)
{
    ...
    mpviz_node::OgmAdapter dynamic(mpviz_node::testing::urban_row("/perception/dynamic_ogm"),
                                    kTf.tf);
    ...
    mpviz_node::OgmAdapter gradient(mpviz_node::testing::urban_row("/perception/gradient_ogm"),
                                     kTf.tf);
...
{
    const auto specs =
        mpviz_node::subscriptions_for(mpviz_node::testing::urban_row("/perception/dynamic_ogm"));
    ...
    auto full_grid = mpviz_node::testing::load_occupancy_grid("ogm_synthetic.yaml");
    auto patch = mpviz_node::testing::load_occupancy_grid_update("ogm_update_synthetic.yaml");
    TfFixture kTf;
    mpviz_node::OgmAdapter a(mpviz_node::testing::urban_row("/perception/dynamic_ogm"), kTf.tf);
