...
uint8_t LerpByte(uint8_t lo, uint8_t hi, float t)
{
    return static_cast<uint8_t>(
        std::lround(static_cast<float>(lo) + (static_cast<float>(hi) - static_cast<float>(lo)) * t));
}
...
void PointCloudAdapter::ingest(const sensor_msgs::msg::PointCloud2& msg, double sim_time_sec)
{
    ...
    for (const auto& f : msg.fields)
    {
        ...
        if (f.name == "x") offX = static_cast<int>(f.offset);
        else if (f.name == "y") offY = static_cast<int>(f.offset);
        else if (f.name == "z") offZ = static_cast<int>(f.offset);
        else if (f.name == "rgb" && offColor < 0) offColor = static_cast<int>(f.offset);
        else if (f.name == "rgba" && offColor < 0) offColor = static_cast<int>(f.offset);
        else if (f.name == "intensity") offIntensity = static_cast<int>(f.offset);
    }
    ...
    std::vector<mpviz::PointCloudPoint> next;
    ...
    for (const auto& rp : raw)
    {
        ...
        switch (tier)
        {
            ...
            {
                const float t = (hi > lo) ? std::clamp((rp.intensity - lo) / (hi - lo), 0.0f, 1.0f)
                                          : 0.5f;
...
void PointCloudAdapter::fill(micropilot::visualization_app::SceneAssembly& out) const
{
