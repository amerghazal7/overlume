                                  ...
                                  double sim_time_sec)
{
    ...
    for (const auto& m : msg.markers)
    {
        ...
        std::optional<mpviz::MarkerPrimitive> single_primitive;
        bool is_fan_out = false;
        switch (m.type)
        {
            case kTypeArrow: single_primitive = mpviz::MarkerPrimitive::ARROW; break;
            case kTypeCube: single_primitive = mpviz::MarkerPrimitive::CUBE; break;
            case kTypeSphere: single_primitive = mpviz::MarkerPrimitive::SPHERE; break;
            case kTypeCylinder: single_primitive = mpviz::MarkerPrimitive::CYLINDER; break;
            case kTypeLineStrip: single_primitive = mpviz::MarkerPrimitive::LINE_STRIP; break;
            case kTypeLineList: single_primitive = mpviz::MarkerPrimitive::LINE_LIST; break;
            case kTypePoints: single_primitive = mpviz::MarkerPrimitive::POINTS; break;
            case kTypeTextViewFacing: single_primitive = mpviz::MarkerPrimitive::TEXT; break;
            case kTypeMeshResource: single_primitive = mpviz::MarkerPrimitive::MESH; break;
            case kTypeTriangleList: single_primitive = mpviz::MarkerPrimitive::TRIANGLE_LIST; break;
            case kTypeCubeList:
        ...
        if (is_fan_out)
        {
            ...
            StoredMarker entry;
            entry.primitive =
                m.type == kTypeCubeList ? mpviz::MarkerPrimitive::CUBE : mpviz::MarkerPrimitive::SPHERE;
        ...
        const mpviz::MarkerPrimitive primitive = *single_primitive;
        StoredMarker entry;
        ...
        if (primitive == mpviz::MarkerPrimitive::LINE_STRIP ||
            primitive == mpviz::MarkerPrimitive::LINE_LIST ||
            primitive == mpviz::MarkerPrimitive::POINTS ||
            primitive == mpviz::MarkerPrimitive::TRIANGLE_LIST)
        {
            const size_t min_points = primitive == mpviz::MarkerPrimitive::TRIANGLE_LIST ? 3
                                       : primitive == mpviz::MarkerPrimitive::POINTS       ? 1
                                                                                            : 2;
        ...
        else
        {
            ...
            if (primitive == mpviz::MarkerPrimitive::MESH)
            {
            ...
            if (primitive == mpviz::MarkerPrimitive::TEXT)
            {
...
void GenericMarkerAdapter::fill(micropilot::visualization_app::SceneAssembly& out) const
{
    for (const auto& [key, entry] : storage_)
    {
        ...
        else
        {
            ...
            g.heading_rad = entry.heading_rad;
            g.scale = entry.scale;
            if (entry.primitive == mpviz::MarkerPrimitive::TEXT)
            {
                g.text = entry.text.c_str();
            }
            else if (entry.primitive == mpviz::MarkerPrimitive::MESH)
            {
