#!/usr/bin/env python3
"""WS-driven `set_theme` end-to-end test: visualization_node + vcam_ws_bridge.
...
that mpviz::set_theme() blends correctly in a unit test (test_theme_transition
...
library-level blend math is correct.
  ...
  1. Launch visualization_node (initial_mode:=3, so it renders+publishes
...
Run (ROS + this repo's ros install sourced first):
    ...
    python3 ros/src/micropilot_visualization_node/test/test_theme_ws.py
...
def main() -> int:
    ...
    try:
        if not _wait_running(viz_proc):
            ...
            return 1
        if not _lifecycle("/visualization_node", "configure"):
            print("FAIL: configure transition failed", file=sys.stderr)
            return 1
        if not _lifecycle("/visualization_node", "activate"):
        ...
        print("PASS: visualization_node WS set_theme end-to-end test passed.")
        return 0
