...
TEST(Callouts, NearestObstacleChipTracksAcrossCameraMove)
{
    ...
    mpviz::Vec3 obstacle{2.0, 1.5, -0.5};
    mpviz::AlertPolygon alert{};
    ...
    pose1.target[2] = -0.5;
    pose1.vfov_deg = 80.0;
    ASSERT_TRUE(mpviz::render_frame(r, pose1, view));

    mpviz_node::Callout c1{};
    const mpviz::Vec3 ego1{pose1.eye[0], pose1.eye[1], pose1.eye[2]};
    ASSERT_TRUE(mpviz_node::BuildNearestCallout(r, &alert, 1, ego1, c1));

    // Second tick: camera orbits to a different eye, same target/obstacle.
    mpviz::CameraPose pose2 = pose1;
    pose2.eye[1] = 3.0;
    ASSERT_TRUE(mpviz::render_frame(r, pose2, view));

    mpviz_node::Callout c2{};
    const mpviz::Vec3 ego2{pose2.eye[0], pose2.eye[1], pose2.eye[2]};
    ASSERT_TRUE(mpviz_node::BuildNearestCallout(r, &alert, 1, ego2, c2));
...
TEST(Callouts, ChipForAnObjectBehindCameraIsSuppressedNotMisdrawn)
{
    ...
    // ReturnsFalse test.
    mpviz::Vec3 behind{pose.eye[0] + (pose.eye[0] - pose.target[0]),
    ...
    mpviz_node::Callout c{};
    const mpviz::Vec3 ego_pos{pose.eye[0], pose.eye[1], pose.eye[2]};
    EXPECT_FALSE(mpviz_node::BuildNearestCallout(r, &alert, 1, ego_pos, c));
...
TEST(Callouts, NoAlertsReturnsFalse)
{
    ...
    mpviz_node::Callout c{};
    EXPECT_FALSE(mpviz_node::BuildNearestCallout(nullptr, nullptr, 0, mpviz::Vec3{}, c));
}
...
//
// This file's own vendored stb_image/stb_image_write (not visual_renderer's
...
TEST(CalloutsGolden, SyntheticFrameWithVisibleCallout720pLowPreset)
{
    ...
    mpviz::Vec3 obstacle{2.0, 1.0, -0.5};
    mpviz::AlertPolygon alert{};
    alert.points = &obstacle;
    alert.point_count = 1;
    alert.severity = 2;
    ...
    mpviz::FrameView view{frame.data(), kW, kH};
    ASSERT_TRUE(mpviz::render_frame(r, pose, view));

    mpviz_node::Callout callout{};
    ASSERT_TRUE(mpviz_node::BuildNearestCallout(r, &alert, 1, scene.ego.position, callout))
    ...
    const std::string golden_path = std::string(MPVIZ_NODE_FIXTURES_DIR) + "/callouts_720p_golden.png";
