...
using micropilot::visualization_app::ClassifyGeoDatum;
using micropilot::visualization_app::GeoAnchor;
using micropilot::visualization_app::GeoAnchorSolver;
using micropilot::visualization_app::GeoDatumOverride;
using micropilot::visualization_app::GreatCircleDistanceM;
using micropilot::visualization_app::kMinAnchorBaselineM;
using micropilot::visualization_app::kMinAnchorSamples;
using micropilot::visualization_app::MapToWgs;
using micropilot::visualization_app::SolveAnchor;
using micropilot::visualization_app::WgsToMap;
...
TEST(GeoAnchor, RoundTripWgsToMapAndBackWithin0p1mOver2km)
{
    ...
    for (auto [dlat, dlon] : {std::pair{0.0, 0.0}, {0.01, 0.0}, {0.0, 0.01}, {-0.015, 0.02}})
    {
        ...
        const mpviz::Vec3 xy = WgsToMap(a, lat, lon);
...
TEST(GeoAnchor, RoundTripHoldsWithNonzeroHeading)
{
    ...
    const mpviz::Vec3 xy = WgsToMap(a, 25.0850, 55.3950);
    const auto [lat2, lon2] = MapToWgs(a, xy);
...
TEST(GeoAnchor, SolveAnchorMatchesBagDerivedMeanPositionAndCoarseHeading)
{
    const std::string path =
        std::string(MPVIZ_NODE_FIXTURES_DIR) + "/geo_anchor_samples_0.csv";
    ...
    for (std::size_t i = 0; i < samples.fixes.size(); ++i)
    {
        ...
        const mpviz::Vec3 got = WgsToMap(anchor, lat, lon);
