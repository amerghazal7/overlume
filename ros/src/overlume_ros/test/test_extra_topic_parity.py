...
Two launches of the real, running overlume_node against the SAME
...
(overlume_node.cpp's timer_callback, ego_anchor.hpp) -- the camera
...
Run (ROS + this repo's ros install sourced first):
    ...
    python3 ros/src/overlume_ros/test/test_extra_topic_parity.py
...
def _run_phase(env: dict, profile_name: str, profile_dir: str):
    ...
    class Fixture(Node):
        def __init__(self):
            ...
            self.look_pub = self.create_publisher(
                Float64MultiArray, "/overlume_node/set_look", 10)
    ...
    try:
        if not _wait_running(node_proc):
            ...
            raise RuntimeError(f"overlume_node exited early:\n{stderr[-2000:]}")
