...
TEST(OgmAdapter, FullGridPopulatesLayerGeometryAndCells) {
    ...
    TfFixture kTf;
    overlume_node::OgmAdapter a(overlume_node::testing::urban_row("/perception/dynamic_ogm"),
                                kTf.tf);
...
TEST(OgmAdapter, PartialUpdatePatchesInPlaceWithoutResizing) {
    ...
    EXPECT_EQ(g->cells[7], overlume_node::kUnknownCell)
...
TEST(OgmAdapter, RoleSelectsLayerKind) {
    ...
    overlume_node::OgmAdapter gradient(
        overlume_node::testing::urban_row("/perception/gradient_ogm"), kTf.tf);
...
TEST(OgmAdapter, OneRowYieldsTwoSubscriptionsAndOneAdapter) {
    const auto specs = overlume_node::subscriptions_for(
        overlume_node::testing::urban_row("/perception/dynamic_ogm"));
