 ...
 *  timer_callback() (OverlumeNode::render_ms_) and, once
 *  governor_enabled_ is true, decides when to drop or recover one
 *  overlume::RenderConfig::quality preset. The node applies that decision by
 *  calling overlume::set_quality() (VM-040's own appended library entry point,
